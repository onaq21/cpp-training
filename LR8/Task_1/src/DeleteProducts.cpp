...
void DeleteProducts(Product*& products, int& size) {
  ...
  cout << "Enter product name to search for deletion (or press enter to skip): ";
  cin.ignore(10000, '\n');
  getline(cin, search_name);
  cout << "Enter quantity to search for deletion (or -1 to skip): ";
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (match) {
      ...
      for (int j = 0; j < size; j++) if (j != i) temp[k++] = products[j];
      delete[] products;
      products = temp;
      size--, i--;
    }
